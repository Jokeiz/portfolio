  ...
  return (
    <section id="process" ref={sectionRef} className="relative py-32 sm:py-40 bg-stage">
      <div className="container-x">
        ...
        {/* Pricing tiers */}
        <div className="exp-reveal mt-32">
            ...
            {[
              {
                ...
                price: '₹6,000',
                priceUsd: '$70',
                desc: 'Single-page site, animated, responsive. 5–7 days. Perfect for product launches, portfolios, link-in-bio upgrades.',
                feat: ['1 page · fully custom', 'Mobile-first', 'Light 3D / motion', 'Hosting + domain setup'],
              },
              {
                tier: 'Marketing site',
                price: '₹18,000',
                priceUsd: '$220',
                featured: true,
                desc: '4–6 page brand site with CMS, blog, and 3D moments. 2–3 weeks. The sweet spot for early-stage brands.',
                feat: ['Up to 6 pages', 'Editable CMS', '3D / interactive sections', 'SEO + analytics'],
              },
              {
                tier: 'Custom build',
                price: 'from ₹40,000',
                priceUsd: '$480+',
                desc: 'Web apps, dashboards, e-commerce, custom tooling. Full-stack. Scoped per project.',
                feat: ['Auth + database', 'Payments (Razorpay/Stripe)', 'Admin panel', '30 days of free care'],
              },
            ].map((t) => (
              <div
                key={t.tier}
                className={`p-8 transition-colors ${
                  t.featured
                    ...
                    : 'bg-ink-900/80 hover:bg-ink-800/80'
                }`}
              >
                <div className="flex items-center justify-between mb-6">
                  <h4 className="font-display text-xl text-slate-150">{t.tier}</h4>
                  {t.featured && (
                ...
                <div className="flex items-baseline gap-2 mb-3">
                  <div className="font-display text-3xl text-slate-150">{t.price}</div>
                  {t.priceUsd && (
                    <div className="font-mono text-xs text-slate-450">/ {t.priceUsd}</div>
                  )}
                </div>
                <p className="text-slate-350 text-sm mb-6 leading-relaxed">{t.desc}</p>
                <ul className="space-y-2">
                  {t.feat.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-sm text-slate-250">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={t.featured ? '#7df9d4' : '#7d889c'} strokeWidth="2.5">
